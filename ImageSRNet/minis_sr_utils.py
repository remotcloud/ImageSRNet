...
class DataSetManager(object):
    def __init__(self, best_individual=None):
    ...
    def getResultImageComp(self, batch_num):
        ...
        """
        # batch_num = 1
        layer2InputData = self.dataSet[0]  # layer1CnnInput,layer1CnnOut,layer2nnInput,layer2CnnOutput
        # 求出神经网络的输出
        layer2DataOneBatchForNN = layer2InputData[batch_num - 1:batch_num]  # 神经网络需要维度[1,4,14,14]
        Layer2SrOutData = self.bestIndividual(layer2DataOneBatchForNN)

        layer2_cnn_network_out_data_forImage = self.dataSet[3][batch_num - 1]
        ...
        if len(cols.shape) == 1:  # cols只是一个一维数组
            for row in range(rowNum):
                for col in range(colNum):
                    cols[row].imshow(layer2_data_input_for_image[col, ...].cpu().data.numpy())
        ...
        for i in range(layer2_cnn_network_out_data_forImage.shape[0]):
            plt.subplot(2, math.ceil(layer2_cnn_network_out_data_forImage.shape[0] / 2), i + 1)
            plt.imshow(layer2_cnn_network_out_data_forImage[i, ...].cpu().data.numpy())
        plt.show()
